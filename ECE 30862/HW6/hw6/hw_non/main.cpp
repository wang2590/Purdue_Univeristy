...
public:
	...
	friend Set operator+(const Set& org, int operand)  // a + 4
	{
		Set y = org;  //a = b
	...
	friend Set operator-(const Set& org, int operand)  //a - 14
	{
	...
	friend Set operator/(const Set& org, Set op){
    	//Set y(this -> length * 32 -1);
    	Set y = org;
    	for(int i = 0; i < y.length; i++)
    	{
    ...
    friend Set operator~(const Set& org)
		{
    		Set y = org;
    		for (int i = 0; i < y.length; i++)
    		{
